...
v1.13.0 以降では、UDP/TCP を連続実行するマトリクススクリプトと、失敗時の復旧案付きレポートを利用できます。
...
マトリクス実行は、内部で `test-windows-nfs-client.ps1` を transport ごとに呼び出します。
Windows の `mount.exe` は portable な NFS version option を公開していないため、Windows Client for NFS ではサーバーログ上の MOUNT v3 / NFSv3 RPC をもって NFSv3 と判定します。
NFSv2 は単体テスト、QNX 4.25 実機確認、必要に応じた Linux/WSL 任意回帰で確認します。
...
- マトリクス実行時は `work\analysis\windows-nfs-client-matrix` 配下へ総合レポートと transport 別レポートを出力します。
...
マトリクス実行では、以下の形式で総合レポートを作成します。

```text
work\analysis\windows-nfs-client-matrix\windows-nfs-client-matrix-YYYYMMDD-HHMMSS.md
```
...
mount は成功するが NFSv3 ログが出ない場合は、前回 mount のキャッシュが残っている可能性があります。`umount Z:` 後にドライブが解放されたことを確認し、必要であれば Windows を再起動してから再実行します。
