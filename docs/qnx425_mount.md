...
v1.13.0 では、QNX 4.25 実機で必要な確認と、ローカル単体テストで代替できる確認を分けて扱います。

| 確認 | ローカル自動確認 | QNX 4.25 実機確認 |
|---|---|---|
| NFSv2 基本手続き | `AllTests` の NFSv2 procedure | `mount_nfs` 後の作成、読込、更新 |
| NFSv2 エラー応答 | `NOENT`、`EXIST`、`NOTDIR` の単体テスト | 実操作で異常終了しないこと |
| 属性応答 | type、mode、uid、gid、size の単体テスト | Phindows などから表示が破綻しないこと |
| 双方向編集 | Windows 側直接編集と NFS 経由編集の単体テスト | QNX 側編集が Windows 側に反映されること |
| 大量コピー | ローカルRPCベンチマークと長時間負荷ループ | 実フォルダコピーの件数、サイズ、内容確認 |
| 大量削除 | `.nfsX*` 互換削除の単体テスト | QNX 側削除後に残骸が残らないこと |
| 階層コピー | サービススモークテストの任意整合性確認 | 深い階層と複数ファイルのコピー |
| symlink / link | `READLINK`、`SYMLINK`、`LINK` の単体テスト | Windows 権限どおりに作成成功または明示失敗すること |

## 制約の切り分け

QNX 側制約:

- `NFSfsys` が起動していない場合、Windows 側が正常でも `mount_nfs` は失敗します。
- QNX 4.25 は古いクライアントのため、NFSv2/UDP 経路を主確認対象にします。
- symlink、特殊ファイル、古いファイル名エンコードは、QNX 側のコピー元の状態に依存します。
- `.nfsX*` 形式の一時名は、削除中のファイルやディレクトリを扱う QNX 側互換挙動として観測されます。

Windows 側制約:

- Windows symlink 作成は、サービスアカウント権限、開発者モード、ファイルシステムに依存します。
- device node など Windows に存在しない特殊ファイルは通常ファイルへ代替しません。
- NTFS は大文字小文字だけが異なる名前の扱いが Linux/QNX と異なるため、診断機能で衝突を確認します。
- ウイルス対策ソフトや Explorer の表示更新タイミングにより、コピー直後の見え方が遅れる場合があります。
