...
この文書は、TinyWinNFS Server v1.14.0 時点のセキュリティ境界、アクセス制限、運用上の注意を整理します。
...
`auto` では、現在のクライアントが送信した AUTH_SYS UID/GID を属性へ反映します。
これにより、QNX、Windows Client for NFS、Linux などがそれぞれ期待する ID を見やすくします。
...
| 設定 | 意味 |
|---|---|
| `file.mode` | 通常ファイルの NFS 属性 mode |
| `directory.mode` | ディレクトリの NFS 属性 mode |
| `exports.N.writable` | NFS 変更操作を許可するか |

NFS mode はクライアント向け属性応答です。
Windows ACL の完全な変換ではありません。
