...
Windows Client for NFS向けの例:

```text
mount -o anon \\<server-host>\<server-mount-name> Z:
```

Linux/WSL向けの例:

```text
sudo mount -t nfs -o vers=3,proto=tcp <server-host>:<server-mount-name> <client-mount-point>
```
...
サービス操作結果には、終了コード、標準出力、標準エラー、失敗理由の目安を表示します。
管理者権限不足、ポート使用中、サービス未登録などは、出力内容から分類して表示します。
サービス操作中は開始、停止、再起動などの操作ボタンを無効化し、多重実行を避けます。
...
診断タブでは、診断パッケージを作成しなくても主要な状態をその場で確認できます。
...
- 設定診断
- export診断
- 大文字小文字衝突、ファイル数、ディレクトリ数、総サイズ

重大度は `情報`、`警告`、`エラー` で絞り込めます。
`設定を開く`、`ログを開く`、`診断出力` から、関連ファイルや診断パッケージへ移動できます。

## ログタブ

ログタブでは、TinyWinNFS サーバーログとマネージャーログを分けて表示します。

サーバーログでは以下を利用できます。

- 検索文字列による絞り込み
- エラー、変更操作、RPC/MOUNT の種別フィルタ
- 手動更新
- 自動更新
- ログファイルの場所を開く

大きいログでは、UIが固まらないようにバックグラウンドで末尾のみを読み込みます。

## 設定インポート/エクスポート

オプションタブから設定をファイルへエクスポートできます。
現在のUI入力内容を検証し、`nfs-server.properties` として保存します。

設定インポートでは、選択した `.properties` を先に検証し、問題がなければ既存設定をバックアップしてから反映します。
不正な設定は反映されません。

`既定に戻す` は、UIを既定値へ戻して保存します。
既存設定がある場合は、保存前にバックアップを作成します。
